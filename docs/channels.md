...
Release. That is a deliberate divergence from cadrumo - a separate product, in its own
repository, that is generated by the same release-channel machinery and so ships the same
channel shapes. Its formula builds a Python virtualenv from a locked sdist cohort, because
cadrumo has no binary channel and its formula has to be the thing that assembles the
product. vaultspec already publishes standalone
