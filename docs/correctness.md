...
re-running the check reports this as a **warning**, and warnings do not raise the exit
code:
...
The `schema` failure above is an error and does. So a pipeline gating on
`vault check all` catches the plan that appeared from nowhere and does not catch the
audit rewritten last night; for that one, read the report rather than the exit status.
