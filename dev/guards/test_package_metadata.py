...
def test_the_readme_states_how_to_get_uv_before_telling_you_to_use_it(
    ...
    runnable instruction on its own. The installer command must appear, and it
    must appear before the first command that depends on it.
    """
    readme = (repo_root / "README.md").read_text(encoding="utf-8")

    installer = readme.find("astral.sh/uv/install")
    assert installer != -1, "README never says how to install uv"
