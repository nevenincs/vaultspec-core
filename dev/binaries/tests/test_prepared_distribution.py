...
#: The two frozen facts `docs/channels.md` states about when binaries stopped
#: installing at launch. Both are historical once the release is cut, so
#: neither expires - which is the shape `dev/guards/test_release_provenance.py`
#: arrived at for the attestation boundary, for the same reason. A caveat with
#: an expiry has to be repaired under release pressure; a named release does
#: not.
_FIRST_OFFLINE = re.compile(
    r"v(?P<version>\d+\.\d+\.\d+) is the first release whose binaries carry "
    r"everything they run"
)

_LAST_INSTALLING = re.compile(
    r"Binaries up to and including v(?P<version>\d+\.\d+\.\d+) install Vaultspec"
)
...
def _channels_prose() -> str:
    ...
    ``mdformat --wrap 88`` decides where these sentences break, so the claim is
    matched against the prose rather than against this week's line boundaries.
    """
    text = (REPO_ROOT / "docs" / "channels.md").read_text(encoding="utf-8")
    return re.sub(r"\s+", " ", text.replace("**", ""))


def _project_version() -> tuple[int, int, int]:
    """The version in ``pyproject.toml``: the newest release cut."""
    raw = (REPO_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    found = re.search(r'^version = "(\d+)\.(\d+)\.(\d+)"', raw, re.MULTILINE)
    assert found is not None, "pyproject.toml declares no X.Y.Z version"
    return int(found[1]), int(found[2]), int(found[3])


def test_the_docs_name_the_release_the_binaries_stopped_installing_at() -> None:
    """A reader on an older binary must not be told it needs no network.

    The claim is true of what this commit builds and false of every binary
    published before it, and the interval between the two is exactly where a
    documentation defect hides. #340 was the same shape pointed the other way.
    """
    prose = _channels_prose()
    assert _FIRST_OFFLINE.search(prose), (
        "docs/channels.md no longer names the release the binaries began "
        "carrying their dependency closure, so the offline claim reads as true "
        "of releases it is false of"
    )
    assert _LAST_INSTALLING.search(prose), (
        "docs/channels.md no longer says which releases install at first "
        "launch, so a user on one has nothing telling them why it needs a "
        "network"
    )


def test_the_documented_offline_boundary_is_self_consistent() -> None:
    """The two versions must describe one boundary, not two.

    Neither may be edited alone. The tie to the present is the only direction
    that can be true: binaries cannot have stopped installing after a release
    that has not been cut.
    """
    prose = _channels_prose()
    first = _FIRST_OFFLINE.search(prose)
    last = _LAST_INSTALLING.search(prose)
    assert first is not None and last is not None

    offline = tuple(int(part) for part in first["version"].split("."))
    installing = tuple(int(part) for part in last["version"].split("."))
    assert offline > installing, (
        f"docs/channels.md says binaries carry everything from v{first['version']} "
        f"but that v{last['version']} and earlier install at launch; the first "
        "offline release cannot precede the last installing one"
    )
    assert installing <= _project_version(), (
        f"docs/channels.md says binaries install at launch up to "
        f"v{last['version']}, which is newer than the version in "
        "pyproject.toml - that release does not exist yet"
    )
