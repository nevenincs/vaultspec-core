...
Usage from CI (``.github/workflows/publish.yml``)::
    ...
    uv run --isolated --no-project --with dist/*.whl dev/smoke/smoke_check.py
...
It is run by PATH in an ``--isolated --no-project`` environment whose only
installed content is the built artifact under test, so it imports nothing
from :mod:`dev` and must keep importing nothing but the standard library and
the distribution it is checking.
"""
...
def check_import() -> None:
    ...
    print(f"PASS: import {vaultspec_core.__name__}")
