"""The ``python -m dev.statistics`` entrypoint wiring the full pipeline.

Running ``python -m dev.statistics`` discovers both transcript corpora, normalizes
every in-window vaultspec-core invocation into one comparable
:class:`~dev.statistics.normalize.models.CallRecord` stream, computes the seven metric
families, and writes the two artifacts into the gitignored ``dev/statistics/out/``
...
runtime; ``--out`` defaults to the repo-relative ``dev/statistics/out``. The declared-
...
from dev.statistics.metrics.capability import (
    CapabilityInventory,
    default_reference_path,
    parse_capability_inventory,
)
from dev.statistics.parsers.claude import ClaudeSource
from dev.statistics.parsers.codex import CodexSource
from dev.statistics.report.render import write_records_jsonl, write_report
