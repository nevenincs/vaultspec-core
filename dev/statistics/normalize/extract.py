...
Where :mod:`dev.statistics.normalize.tokenize` reduces a raw command string to the
segments that invoke ``vaultspec-core``, this module parses each segment into a
canonical :class:`~dev.statistics.normalize.models.CallRecord`. For every segment it
...
from dev.statistics.normalize.tokenize import EXECUTABLE, candidate_segments
...
class ParsedCommand:
    ...
    Attributes:
        ...
        unresolved: ``True`` when the segment could not be tokenized under either
            ...
            :attr:`~dev.statistics.normalize.exit_status.ExitStatus.UNKNOWN` rather
...
class CommandContext:
    ...
    Everything on a :class:`~dev.statistics.normalize.models.CallRecord` that cannot
...
def parse_command(
    ...
    Runs stage one (:func:`~dev.statistics.normalize.tokenize.candidate_segments`) to
...
def extract_records(
    ...
    :class:`~dev.statistics.normalize.models.CallRecord`. The command itself survives
    only as :attr:`~dev.statistics.normalize.models.CallRecord.command_hash`; no raw
