...
:class:`~dev.statistics.parsers.base.TranscriptSource` protocol over the Codex
...
#: A decoded JSON object - the shape every rollout line parses into.
...
class CodexSource:
    ...
    @staticmethod
    def _collect_outputs(objects: list[_JSONObject | None]) -> dict[str, int]:
    ...
    @staticmethod
    def _output_text(output: _JSONValue | None) -> str:
    ...
    @staticmethod
    def _decode_arguments(arguments: object) -> _JSONObject | None:
        ...
        if not isinstance(arguments, str):
            return None
        try:
            decoded: _JSONValue = json.loads(arguments)
        except ValueError:
            return None
    ...
    @staticmethod
    def _subagent_role(payload: _JSONObject) -> str | None:
    ...
    def _exit_status(exit_code: int | None, subcommand: tuple[str, ...]) -> ExitStatus:
        ...
        Returns:
            The mapped :class:`~dev.statistics.normalize.exit_status.ExitStatus`.
        """
