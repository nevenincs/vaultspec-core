...
:class:`~dev.statistics.parsers.base.TranscriptSource` protocol over the Claude Code
...
:func:`~dev.statistics.normalize.extract.extract_records`; this adapter only derives
...
#: A decoded JSON scalar, array, or object value, recursively.
type _JSONValue = (
    str | int | float | bool | list[_JSONValue] | dict[str, _JSONValue] | None
)

#: A decoded JSON object - the shape every transcript line parses into.
...
def _load_object(line: str) -> _JSONObject | None:
    ...
    if not line:
        return None
    try:
        obj: _JSONValue = json.loads(line)
    except ValueError:
        return None
    return obj if isinstance(obj, dict) else None


def _flatten_content(content: _JSONValue | None) -> str:
...
class ClaudeSource:
    ...
    @staticmethod
    def _attribute_cost(
        records: list[CallRecord], message: _JSONObject
    ) -> Iterator[CallRecord]:
    ...
    def _shell_calls(self, message: _JSONObject) -> list[_JSONObject]:
    ...
    @staticmethod
    def _command_of(entry: _JSONObject) -> str | None:
    ...
    @staticmethod
    def _message_usage_total(message: _JSONObject) -> int | None:
    ...
    def _exit_status(result: _Result | None, subcommand: tuple[str, ...]) -> ExitStatus:
        ...
        Returns:
            The inferred :class:`~dev.statistics.normalize.exit_status.ExitStatus`.
        """
        if result is None:
