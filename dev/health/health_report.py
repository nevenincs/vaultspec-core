...
Usage:
    uv run --no-sync python dev/health/health_report.py [--top N] [--fast] [--census]
...
#: Report every function above this score. It is deliberately far below the
#: ratchet in ``[tool.complexipy]`` so the table shows the burndown queue, not
#: just the functions already failing; a guard asserts the ordering holds.
COGNITIVE_REPORT_FLOOR = 10
...
def report_strict_types(top: int) -> None:
    _section(
        ...
        "gating since the burndown reached zero; ty remains the fast gate",
    )
