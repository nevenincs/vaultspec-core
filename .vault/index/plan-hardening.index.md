---
generated: true
tags:
  - '#index'
  - '#plan-hardening'
date: '2026-05-05'
related:
  - '[[2026-05-05-plan-hardening-adr]]'
  - '[[2026-05-05-plan-hardening-exec]]'
  - '[[2026-05-05-plan-hardening-plan]]'
  - '[[2026-05-05-plan-hardening-research]]'
  - '[[2026-05-06-plan-hardening-adr]]'
---

# `plan-hardening` feature index

Auto-generated index of all documents tagged with `#plan-hardening`.

## Documents

### adr

- `2026-05-05-plan-hardening-adr` - `plan-hardening` adr: plan-hardening natural-language convention | (**status:** `proposed`)
- `2026-05-06-plan-hardening-adr` - `plan-hardening` adr: vault plan CLI surface | (**status:** `proposed`)

### exec

- `2026-05-05-plan-hardening-exec` - `plan-hardening` exec summary

### plan

- `2026-05-05-plan-hardening-plan` - `plan-hardening` plan

### research

- `2026-05-05-plan-hardening-research` - `plan-hardening` research: umbrella research
