---
generated: true
tags:
  - '#index'
  - '#template-annotation-sanitization'
date: '2026-05-15'
modified: '2026-06-13'
related:
  - '[[2026-05-15-template-annotation-sanitization-adr]]'
  - '[[2026-05-15-template-annotation-sanitization-plan]]'
  - '[[2026-05-15-template-annotation-sanitization-research]]'
---

# `template-annotation-sanitization` feature index

Auto-generated index of all documents tagged with `#template-annotation-sanitization`.

## Documents

### adr

- `2026-05-15-template-annotation-sanitization-adr` - template-annotation-sanitization adr: explicit generated annotation sanitation | status: accepted

### plan

- `2026-05-15-template-annotation-sanitization-plan` - template-annotation-sanitization plan: implementation

### research

- `2026-05-15-template-annotation-sanitization-research` - template-annotation-sanitization research: generated annotation lifecycle
