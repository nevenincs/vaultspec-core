...
- [ ] `P01.S01` - Add the canonical Code-stands-alone mandate bullet (removable scaffolding, one-way reference direction, trailer carve-out) beside the Comments mandate; `src/vaultspec_core/builtins/system/01-core.md`.
- [ ] `P01.S02` - Add the one-sentence removable-harness characterization with one-way reference direction where the .vault store is introduced; `src/vaultspec_core/builtins/system/03-vaultspec.md`.
...
- [ ] `P03.S07` - Propagate builtin edits to the deployed mirror with vaultspec-core sync and confirm spec doctor is clean; `.vaultspec/`.
