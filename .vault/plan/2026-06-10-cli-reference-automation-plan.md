...
- [x] `P01.S03` - Correct the vault add --feature required-marker annotation to match live --help and grep the reference for any other stale required-markers (P03 doc gap); `src/vaultspec_core/builtins/reference/cli.md`.
...
- [x] `P02.S06` - GATED on the ADR deciding build, document the generator as the canonical reference-update path in the CLI rule (D6 deferral); `src/vaultspec_core/builtins/rules/vaultspec-cli.builtin.md`.
