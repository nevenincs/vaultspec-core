...
- [ ] `P01.S01` - Add a removal-milestone marker to the legacy template-name fallback so the ref-audit.md grace path is scheduled for removal one release after the rename, keeping the existing five fallback tests green (REVIEW-005 fallback); `src/vaultspec_core/vaultcore/hydration.py`.
...
- [ ] `P02.S04` - Produce a decision ADR weighing a Typer-surface auto-generator for the bundled reference against the existing hand-authored-plus-drift-guard approach, deciding whether to build it (D6 deferral); `.vault/adr`.
