...
- [x] `P03.S22` - implement vault link add with target resolution, dangling refusal behind force, and dry-run preview; `src/vaultspec_core/cli/link_cmd.py`.
...
- [x] `P03.S27` - regenerate the bundled CLI reference and propagate provider sync; `.vaultspec/rules/reference/cli.md`.
...
- [x] `P04.S30` - invalidate or refresh the graph cache from every mutating verb that touches vault documents; `src/vaultspec_core/cli/`.
- [x] `P04.S31` - add cache correctness tests proving a stale cache is never trusted; `src/vaultspec_core/graph/tests/test_cache.py`.
- [x] `P04.S32` - add 500 and 5000 document scale benchmarks with generous thresholds; `src/vaultspec_core/graph/tests/test_scale.py`.
- [x] `P04.S33` - register the dedicated benchmark marker; `pyproject.toml`.
