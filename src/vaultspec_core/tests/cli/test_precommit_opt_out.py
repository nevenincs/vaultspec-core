...
class TestScaffoldHonoursTheDeclaration:
    ...
    def test_explicitly_enabled_workspace_is_scaffolded(self, tmp_path: Path) -> None:
        ...
        assert _scaffold_precommit(tmp_path) == [(_CONFIG, "precommit")]
        assert (tmp_path / _CONFIG).exists()
    ...
    def test_dry_run_reports_nothing_when_declined(self, tmp_path: Path) -> None:
        ...
        assert _scaffold_precommit(tmp_path, dry_run=True) == []
    ...
    def test_existing_config_is_left_alone(self, tmp_path: Path) -> None:
        ...
        assert _scaffold_precommit(tmp_path) == []
    ...
    def test_resolver_plans_no_precommit_repair(self, factory) -> None:
...
class TestOptOutSurvivesInstall:
    def test_upgrade_does_not_resurrect_the_declined_config(self, factory) -> None:
    ...
    def test_install_over_a_committed_declaration_writes_nothing(self, factory) -> None:
...
class TestManagedGitignoreEntry:
    def test_declined_config_is_ignored(self, factory) -> None:
        factory.install()
        _decline(factory.root)

        assert f"/{_CONFIG}" in get_recommended_entries(factory.root)

    def test_wanted_config_is_not_ignored(self, factory) -> None:
    ...
    def test_entry_reaches_the_written_block(self, factory) -> None:
...
class TestCliVerbs:
    def test_disable_writes_the_declaration(self, runner, factory) -> None:
    ...
    def test_enable_clears_the_declaration(self, runner, factory) -> None:
        from vaultspec_core.tests.cli.conftest import run_vaultspec
    ...
    def test_disable_is_idempotent(self, runner, factory) -> None:
    ...
    def test_disable_reports_updated_on_first_call(self, runner, factory) -> None:
        import json
