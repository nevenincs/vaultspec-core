...
class TestSpecCliHelp:
    ...
    def test_mcps_help(self, runner: CliRunner, synthetic_project: Path):
        result = runner.invoke(
    ...
    def test_rules_help(self, runner: CliRunner, synthetic_project: Path):
        result = runner.invoke(
    ...
    def test_skills_help(self, runner: CliRunner, synthetic_project: Path):
        result = runner.invoke(
            app, ["--target", str(synthetic_project), "spec", "skills", "--help"]
        )
        assert result.exit_code == 0

    def test_system_help(self, runner: CliRunner, synthetic_project: Path):
        result = runner.invoke(
    ...
    def test_hooks_help(self, runner: CliRunner, synthetic_project: Path):
        result = runner.invoke(
...
class TestSpecCliFunctional:
    ...
    def test_rules_list_output(self, runner: CliRunner, synthetic_project: Path):
        result = runner.invoke(
            app, ["--target", str(synthetic_project), "spec", "rules", "list"]
        )
        assert result.exit_code == 0

    def test_hooks_list_empty(self, runner: CliRunner, tmp_path: Path):
...
class TestSpecCliDispatchRouting:
    ...
    def test_rules_list_handler(self, synthetic_project: Path):
        from ...core import rules_list

        rules_list()

    def test_rules_add_handler(self, tmp_path: Path):
