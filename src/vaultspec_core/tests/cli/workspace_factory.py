...
class WorkspaceFactory:
    ...
    def outdated_vaultspec_rules(self, provider: str) -> Self:
        """Overwrite every source-backed synced rule to simulate stale content.

        The staleness collector decides drift by comparing each *source-backed*
        rule against its rendered expectation; the synthesized system rule
        (:data:`~vaultspec_core.core.system.SYSTEM_BUILTIN_RULE`) has no source
        and is deliberately ignored. Overwriting an arbitrary ``glob`` result
        could therefore land on that ignored file and produce no drift signal at
        all - a nondeterministic no-warning state driven by filesystem
        enumeration order. This overwrites all rule files except the system rule
        in sorted order so the drift is deterministic, and raises if it finds
        nothing to make stale rather than silently no-oping.
        """
        from vaultspec_core.core.system import SYSTEM_BUILTIN_RULE

        rules_dir = _provider_dir(self.root, provider) / "rules"
        targets = sorted(
            md for md in rules_dir.glob("*.md") if md.name != SYSTEM_BUILTIN_RULE
        )
        if not targets:
            msg = (
                f"outdated_vaultspec_rules('{provider}') found no source-backed "
                f"rule to make stale under {rules_dir}; the workspace must be "
                "installed with synced provider rules before staling them."
            )
