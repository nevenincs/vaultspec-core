...
def test_marker_block_yields_expected_verb_paths(vault_root: Path) -> None:
...
def test_verb_entry_carries_flag_schema_and_json_support(vault_root: Path) -> None:
...
def test_denylisted_verbs_absent_from_catalog(vault_root: Path) -> None:
...
def test_denylisted_verbs_never_ranked(vault_root: Path) -> None:
...
def test_search_ranks_a_known_verb_for_its_intent(vault_root: Path) -> None:
