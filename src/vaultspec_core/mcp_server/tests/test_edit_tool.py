...
async def _create_adr(client: Client, feature: str) -> str:
...
async def _edit(client: Client, operations: list[dict[str, Any]]) -> Any:
    result = await client.call_tool("edit", {"operations": operations})
    return data_of(result)


async def test_edit_set_body_returns_post_write_hash(vault_root: Path) -> None:
...
async def test_edit_blob_hash_conflict(vault_root: Path) -> None:
...
async def test_edit_section_not_found(vault_root: Path) -> None:
...
async def test_edit_post_write_hash_chains_across_documents(vault_root: Path) -> None:
...
async def test_edit_empty_batch_raises_protocol_error(vault_root: Path) -> None:
...
async def test_edit_unresolvable_target_fails_item(vault_root: Path) -> None:
