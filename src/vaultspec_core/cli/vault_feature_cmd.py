...
def _register_feature_archive_commands(feature_app: _typer.Typer) -> None:
    ...
    def cmd_feature_archive(  # pyright: ignore[reportUnusedFunction]
        ...
        try:
            result = archive_feature(
                _get_ctx().target_dir, feature_tag, dry_run=dry_run
            )
        ...
        if dry_run:
            console.print(
                f"[yellow]Dry-run: Previewing feature archive for "
            ...
            if result["archived_count"] == 0:
                console.print(
                    f"[dim]No documents found for feature '{feature_tag}'.[/dim]"
                )
            else:
                console.print(
    ...
    def cmd_feature_unarchive(  # pyright: ignore[reportUnusedFunction]
        ...
        if json_output:
            ...
            unarchive_status = (
                "restored"
                if result["unarchived_count"] and not dry_run
                else "unchanged"
            )
            typer.echo(
                json.dumps(
        ...
        if dry_run:
            ...
            if result["unarchived_count"] == 0:
                console.print(
                    "[dim]No archived documents found for feature "
                    f"'{feature_tag}'.[/dim]"
                )
            else:
                console.print(
...
def _register_feature_rename_command(feature_app: _typer.Typer) -> None:
    ...
    def cmd_feature_rename(  # pyright: ignore[reportUnusedFunction]
        ...
        if json_output:
            ...
            rename_status = (
                "updated"
                if (result["renamed_count"] > 0 and not dry_run)
                else "unchanged"
            )
            typer.echo(
                json.dumps(
        ...
        if dry_run:
            ...
            if result["renamed_count"] == 0:
                console.print(
                    f"[dim]No documents found for feature '{old_feature}'.[/dim]"
                )
            else:
                console.print(
