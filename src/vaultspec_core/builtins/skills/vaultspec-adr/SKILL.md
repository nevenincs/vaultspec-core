...
- Check coverage per the `vaultspec-discovery` rule: search for the decision, then list
  ...
  When search gives no such verdict (it declined, or replied "no record that was read
  answers this", `none_read_answers`), the ADR listing plus a targeted grep of
  `.vault/adr/` decide coverage. Read covering decisions whole, then their evidence.
