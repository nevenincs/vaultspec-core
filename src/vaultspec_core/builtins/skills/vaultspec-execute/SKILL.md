...
- **One Step Record per completed Step.** The executor writes a Step Record to
  ...
  path touched, no prose. The Step row already states the intent and the commit
  carries the diff, so a record that narrates them is duplicated context, not
  evidence. Add `## Notes` only on exception (data loss, skipped work, a scaffold
  left in code, a persistent failure). The verb machine-fills the tier-conditional filename from
