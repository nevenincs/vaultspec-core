...
def _make_workspace(tmp_path: Path) -> tuple[Path, Path]:
    ...
    The workspace is built inside pytest's per-test ``tmp_path`` so a run
    never writes scratch state into the tree it is imported from.
    """
    mcps_dir = tmp_path / ".vaultspec" / "mcps"
    mcps_dir.mkdir(parents=True, exist_ok=True)
    return tmp_path, mcps_dir


def _init_context(path: Path) -> WorkspaceContext:
...
class TestCollectMcpServers:
    def test_empty_directory(self, tmp_path: Path):
        path, _mcps_dir = _make_workspace(tmp_path)
        try:
            _init_context(path)
    ...
    def test_single_builtin(self, tmp_path: Path):
    ...
    def test_custom_definition(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
    ...
    def test_parse_error_captured_in_warnings(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
    ...
    def test_non_object_json_rejected(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
    ...
    def test_mixed_builtin_and_custom(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
    ...
    def test_custom_shadows_builtin_in_collect(self, tmp_path: Path):
    ...
    def test_empty_stem_filenames_skipped(self, tmp_path: Path):
...
@pytest.mark.unit
class TestMcpList:
    def test_shadowed_definition_shows_custom(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
            (mcps_dir / "srv.builtin.json").write_text(
    ...
    def test_lists_with_source_classification(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
...
@pytest.mark.unit
class TestMcpAdd:
    def test_creates_definition_file(self, tmp_path: Path):
        path, _mcps_dir = _make_workspace(tmp_path)
        try:
            _init_context(path)
    ...
    def test_created_file_stays_within_mcps_dir(self, tmp_path: Path):
    ...
    def test_rejects_builtin_suffix(self, tmp_path: Path):
        path, _mcps_dir = _make_workspace(tmp_path)
        try:
            _init_context(path)
            from vaultspec_core.core.exceptions import VaultSpecError
    ...
    def test_rejects_non_dict_config(self, tmp_path: Path):
        path, _mcps_dir = _make_workspace(tmp_path)
        try:
            _init_context(path)
            from vaultspec_core.core.exceptions import VaultSpecError
...
class TestMcpRemove:
    ...
    def test_removes_builtin_file(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
    ...
    def test_custom_removed_before_builtin(self, tmp_path: Path):
    ...
    def test_removed_file_was_within_mcps_dir(self, tmp_path: Path):
    ...
    def test_rejects_traversal_in_remove(self, tmp_path: Path):
        path, _mcps_dir = _make_workspace(tmp_path)
        try:
            _init_context(path)
            from vaultspec_core.core.exceptions import VaultSpecError
    ...
    def test_rejects_empty_name_in_remove(self, tmp_path: Path):
        path, _mcps_dir = _make_workspace(tmp_path)
        try:
            _init_context(path)
            from vaultspec_core.core.exceptions import VaultSpecError
...
@pytest.mark.unit
class TestMcpSync:
    def test_creates_mcp_json_from_scratch(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
            config = {"command": "uv", "args": ["run", "test"]}
    ...
    def test_idempotent_sync(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
            config = {"command": "uv", "args": ["run", "test"]}
            (mcps_dir / "test-srv.builtin.json").write_text(
    ...
    def test_sync_records_canonical_action_strings(self, tmp_path: Path):
    ...
    def test_preserves_user_entries(self, tmp_path: Path):
        ...
        try:
            ...
            user_config: dict[str, dict[str, dict[str, str | list[str]]]] = {
    ...
    def test_warns_on_diff_without_force(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
            (mcps_dir / "srv.builtin.json").write_text(
                json.dumps({"command": "new"}), encoding="utf-8"
    ...
    def test_force_overwrites_diff(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
...
class TestMcpUninstall:
    def test_removes_managed_entries(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
            (mcps_dir / "managed.builtin.json").write_text(
                json.dumps({"command": "uv"}), encoding="utf-8"
    ...
    def test_unowned_entry_survives_without_registry_or_ownership(self, tmp_path: Path):
        path = tmp_path
        try:
...
class TestUninstallRemovesCustomMcps:
    def test_full_uninstall_removes_custom_mcp_entries(self, tmp_path: Path):
...
class TestMcpSyncPrune:
    ...
    def test_prune_removes_orphan_managed_entry(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
    ...
    def test_prune_default_false_leaves_orphans(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
    ...
    def test_prune_preserves_user_added_entries(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
            (mcps_dir / "managed.builtin.json").write_text(
                json.dumps({"command": "uv"}), encoding="utf-8"
            )
            user_data: dict[str, dict[str, dict[str, str | list[str]]]] = {
    ...
    def test_user_added_with_shared_name_never_taken_over(self, tmp_path: Path):
        ...
        try:
            ...
            existing: dict[str, dict[str, dict[str, str]] | list[str]] = {
    ...
    def test_name_intersection_without_legacy_evidence_remains_external(
        self, tmp_path: Path
    ):
    ...
    def test_managed_key_persisted_across_syncs(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
            (mcps_dir / "alpha.builtin.json").write_text(
                json.dumps({"command": "uv"}), encoding="utf-8"
    ...
    def test_prune_skips_entries_whose_source_failed_to_parse(self, tmp_path: Path):
    ...
    def test_prune_preserves_user_top_level_keys(self, tmp_path: Path):
    ...
    def test_round_trip_install_uninstall_via_sync_provider(self, tmp_path: Path):
...
def _native_servers(path: Path, provider: str) -> dict[str, dict[str, object]]:
...
class TestProviderNativeMcpEnrollment:
    def test_explicit_public_seam_writes_all_native_targets(self, tmp_path: Path):
        path, mcps_dir = _make_workspace(tmp_path)
        try:
            definition = {
